...
class TransacaoController extends Controller
{
    ...
    /**
     * Relatório de transações (resumo por período)
     */
    public function relatorio(Request $request): JsonResponse
    {
        $validator = Validator::make($request->all(), [
            ...
            'data_inicio.date' => 'Digite uma data de início válida',
            'data_fim.required' => 'A data de fim é obrigatória',
            'data_fim.date' => 'Digite uma data de fim válida',
            'data_fim.after_or_equal' => 'A data de fim deve ser posterior à data de início',
        ]);
        ...
        try {
            $transacoes = Transacao::whereBetween('data_transacao', [
                $request->data_inicio, 
                $request->data_fim
            ])->get();

            $entradas = $transacoes->where('tipo_transacao', 'entrada')->sum('valor');
            $saidas = $transacoes->where('tipo_transacao', 'saida')->sum('valor');
